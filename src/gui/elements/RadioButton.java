...
public class RadioButton extends Element {
	...
	void setEnabled(boolean enabled) { // used by RadioButtonGroup
		this.enabled = enabled;
		activationCalc.setHovered(enabled);
		updateSlideCalc(contains(getMousePosition()));
	}

	void setInitially(boolean isOn) { // used by RadioButtonGroup
		enabled = isOn;
		activationCalc.setInitially(isOn);
		slideCalc.setInitially(isOn);
	}
